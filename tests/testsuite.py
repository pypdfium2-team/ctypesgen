...
def _generate_stdlib(*extra_args):
    args = ["--system-headers", "stdlib.h", "-l", STDLIB_NAME, "--symbol-rules", r"if_needed=__\w+", *extra_args]
    # on macOS, the functions don't appear to be in stdlib.h directly, so we include transitive members
    if sys.platform.startswith("darwin"):
        args.append("--all-headers")
    return generate(header=None, args=args)
...
class VariadicFunctionTest(TestCaseWithCleanup):
    ...
    @classmethod
    def setUpClass(cls):
        args = ["--system-headers", "stdio.h", "-l", STDLIB_NAME, "--symbol-rules", r"if_needed=__\w+"]
        # on macOS, the functions don't appear to be in stdio.h directly, so we include transitive members
        if sys.platform.startswith("darwin"):
            args.append("--all-headers")
        cls.module = generate(header=None, args=args)
...
class APITest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # on macOS, the functions don't appear to be in stdio.h directly, so we include transitive members
        if sys.platform.startswith("darwin"):
            args["all_headers"] = True
        ctg_main.api_main(args)
...
""" % (extra, STDLIB_NAME, repr(self.outpath).replace(str(CTYPESGEN_DIR), "."))
...
class AutostringsTest(TestCaseWithCleanup):
    ...
    @classmethod
    def setUpClass(cls):
        args = ["--system-headers", "string.h", "wchar.h", "-l", STDLIB_NAME, "--string-template", str(STRING_TEMPLATE)]
        # on macOS, the functions don't appear to be in string.h/wchar.h directly, so we include transitive members
        if sys.platform.startswith("darwin"):
            args.append("--all-headers")
        cls.module = generate(header=None, args=args)
