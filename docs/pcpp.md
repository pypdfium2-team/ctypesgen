...
On the other hand, as pcpp maintainer Niall Douglas [points out](https://github.com/ned14/pcpp/issues/85#issuecomment-1860619214),
"if you have to bother doing that, you might as well have it \[gcc\] do the preprocessing too".
...
As of this writing, the matching rules are loose, i.e. using just names rather than full paths, but in case this changes and you get an empty output, pass `--all-headers` to ctypesgen to forego symbol filtering.
